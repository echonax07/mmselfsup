...
# srun --ntasks=2 --gres=gpu:2 --kill-on-bad-exit=1 --cpus-per-task=10 python tools/train.py $1 --launcher slurm
...
# deactivate
echo starting only_finetune

cd /home/m32patel/projects/rrg-dclausi/ai4arctic/m32patel/mmsegmentation

source ~/env_mmsegmentation/bin/activate

echo "Config file: $1"

# srun --ntasks=2 --gres=gpu:2  --kill-on-bad-exit=1 --cpus-per-task=6 python tools/train.py $2 --launcher slurm --cfg-options model.backbone.init_cfg.checkpoint=${CHECKPOINT}
...
python tools/test.py $1 $CHECKPOINT_mmseg --out work_dirs/$base_name_mmseg/ --show-dir work_dirs/$base_name_mmseg/