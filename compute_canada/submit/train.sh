...
cd $HOME/projects/def-y2863che/ai4arctic/$USER/mmselfsup
...
echo "Config file: $1"
# srun --ntasks=2 --gres=gpu:2  --kill-on-bad-exit=1 --cpus-per-task=6 python tools/train.py $1 --launcher slurm --resume
# srun --ntasks=2 --gres=gpu:2  --kill-on-bad-exit=1 --cpus-per-task=12 python tools/train.py $1 --launcher slurm
srun --ntasks=2 --gres=gpu:2  --kill-on-bad-exit=1 --cpus-per-task=6 python tools/train.py $1 --launcher slurm
